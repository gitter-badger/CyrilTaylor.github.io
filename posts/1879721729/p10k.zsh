# Generated by Powerlevel10k configuration wizard on 2019-10-08 at 14:11 CST.
# Based on romkatv/powerlevel10k/config/p10k-lean.zsh, checksum 39152.
# Wizard options: nerdfont-complete + powerline, small icons, lean, time, 2 lines,
# solid, compact, many icons, concise.
...
() {
  ...
  function my_git_formatter() {
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      ...
      res+="${meta}${(g::)POWERLEVEL9K_VCS_TAG_ICON}"
      where=${(V)VCS_STATUS_TAG}
    else
      res+="${meta}${(g::)POWERLEVEL9K_VCS_COMMIT_ICON}"
    ...
    if [[ -n ${VCS_STATUS_REMOTE_BRANCH} ]]; then
      res+="${meta}-->${(g::)POWERLEVEL9K_VCS_REMOTE_BRANCH_ICON}${clean}${(V)VCS_STATUS_REMOTE_BRANCH//\%/%%}"  # escape %
    fi
